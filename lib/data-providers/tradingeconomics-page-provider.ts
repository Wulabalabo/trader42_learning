...
export const tradingEconomicsPageProvider: IndicatorDataProviderAdapter = {
  ...
  async getSnapshot({ seriesKey, fetchImpl = fetch }) {
    ...
    const match = text.match(/(?:increased\s+to|decreased\s+to|was unchanged at|remained at)\s+([\d.]+)\s+points\s+in\s+([A-Za-z]+)\s+from\s+([\d.]+)\s+points\s+in\s+([A-Za-z]+)\s+of\s+(\d{4})/i);

    if (!match) {
      return undefined;
    }

    const [, latestValue, latestMonth, previousValue, , year] = match;
