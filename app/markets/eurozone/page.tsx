...
export default async function EurozoneMarketPage() {
  const themes = getAllThemeSeeds('eurozone');

  return (
    <TerminalWorkspace
      leftRail={<TerminalDirectory activeKey="eurozone-market" activeScopes={['欧元区']} />}
      rightRail={
          ...
          <Card>
            <CardBody className="space-y-3">
              ...
              <div className="space-y-2">
                {eurozoneMarketContent.teachingSteps.map((step, index) => (
    ...
    >
      <div className="space-y-3">
        <Card>
          <CardBody className="space-y-4">
            <div className="border-b border-[color:var(--border-subtle)] pb-3">
              <p className="text-[12px] font-medium tracking-[0.12em] text-[color:var(--text-secondary)]">{eurozoneMarketContent.eyebrow}</p>
            </div>

            <div className="space-y-3">
              <h1 className="text-[2.2rem] font-semibold tracking-[0.04em] text-[color:var(--text-primary)] sm:text-[2.7rem]">{eurozoneMarketContent.title}</h1>
              <p className="max-w-3xl text-[15px] leading-7 text-[color:var(--text-secondary)]">{eurozoneMarketContent.hero}</p>
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardBody className="space-y-3">
            <div className="space-y-2 border-b border-[color:var(--border-subtle)] pb-3">
              ...
              <p className="text-[13px] font-semibold tracking-[0.08em] text-[color:var(--text-primary)]">{eurozoneMarketContent.bigPicture.title}</p>
              <p className="text-[14px] leading-6 text-[color:var(--text-secondary)]">{eurozoneMarketContent.bigPicture.description}</p>
            </div>

            <div className="grid gap-2 lg:grid-cols-2">
              {eurozoneMarketContent.bigPicture.items.map((item) => (
        ...
        <Card>
          <CardHeader>
            <p className="text-[13px] font-semibold tracking-[0.08em] text-[color:var(--text-primary)]">{eurozoneMarketContent.themeSectionTitle}</p>
            <p className="mt-2 text-[15px] leading-7 text-[color:var(--text-secondary)]">{eurozoneMarketContent.themeSectionDescription}</p>
          </CardHeader>
          <CardBody>
            <div className="grid gap-3 lg:grid-cols-2">
              {themes.map((theme) => (
                <Link key={theme.slug} href={getThemeHref('eurozone', theme.slug)} className="block h-full">
