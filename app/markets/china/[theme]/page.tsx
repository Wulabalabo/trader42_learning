...
export async function generateMetadata({ params }: MarketThemePageProps): Promise<Metadata> {
  ...
  const themeSeed = getThemeSeedBySlug('china', theme);
...
export async function generateStaticParams() {
  return getAllThemeSeeds('china').map((theme) => ({ theme: theme.slug }));
}

export default async function ChinaMarketThemePage({ params }: MarketThemePageProps) {
  const { theme } = await params;
  const themeData = await getThemeBySlug('china', theme, { loadIndicator: getCachedIndicatorDataBySlugSafe });
  ...
  return (
    <TerminalWorkspace
      leftRail={<TerminalDirectory activeTheme={themeData.slug} activeScopes={['中国']} />}
      rightRail={
